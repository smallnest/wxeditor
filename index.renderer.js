...
(function () {
  ...
  const SVG_DECO = {
    ...
    // ── H2 ──
    'icon-tag': (c) => `<svg width="20" height="20" viewBox="0 0 18 18" xmlns="http://www.w3.org/2000/svg" style="flex-shrink:0;"><path d="M3 2h5l7 7-7 7H3V2z" fill="${c}" opacity="0.25"/><path d="M3 2h5l7 7-7 7H3V2z" fill="none" stroke="${c}" stroke-width="1.8" stroke-linejoin="round"/></svg>`,
  ...
  window.renderMarkdown = function (md, settings) {
    ...
    const wrapperStyle = `color:${g.ink}; background:${g.bg || 'transparent'}; font-family:${fontFamily}; font-size:${settings.p.fontSize}px; line-height:${settings.p.lineHeight}; max-width:100%; word-wrap:break-word; overflow-wrap:break-word; padding:16px 16px 40px;`;
